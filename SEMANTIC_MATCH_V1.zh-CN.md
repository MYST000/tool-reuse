...
语义命中固定返回 `reusable=false`。它只负责召回候选，不能像 exact-v2 一样直接阻断工具执行。

## 当前支持的工具

- terminal curl：method、host、path、query 和 pipe 后处理构造语义文本；
- `browser_navigate`：从 canonical URL 的 host/path/query 构造语义文本；
- `browser_get_content` 历史记录：从 observation 的 `<url>` 和 `<webpage_content>` 提取页面正文，最多索引 12,000 字符；空正文不导入；
- 工具名包含 `search` 的工具：递归提取 `query/search_query/q/keywords/text`。

候选默认必须属于同一 `operation_kind`：

- `curl_http` 只匹配 curl 历史；
- `browser_navigate_url` 匹配 browser navigate 及其页面正文历史；
- `web_search` 只匹配搜索历史。
...
- success、TTL、过期时间；
- 脱敏后的 tool input；
- tool response、response preview/hash。

provider/model 会参与索引隔离。数据库权限每次打开时校正为 `0600`。
