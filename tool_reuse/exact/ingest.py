...
from .policy import response_status
        ...
        with jsonl.open("r", encoding="utf-8") as handle:
            for line_no, line in enumerate(handle, 1):
                ...
                try:
                    ...
                    raise ValueError(f"Invalid JSON at {jsonl}:{line_no}: {exc}") from exc
...
def entry_from_record(source: dict[str, Any], source_path: str) -> ExactEntry | None:
    ...
    if not isinstance(tool_name, str) or not isinstance(tool_input, dict) or not isinstance(tool_response, dict):
        return None
    exact_call = normalize_exact_call(tool_name, tool_input)
    ...
    expires_at_epoch = (
        observed_at_epoch + exact_call.ttl_seconds if observed_at_epoch is not None else None
    )
    response_text = extract_response_text(tool_response)
    return ExactEntry(
        record_key=str(source.get("record_key") or sha256_text(json.dumps(source, sort_keys=True))),
        source_path=source_path,
        exact_call=exact_call,
        started_at=source.get("started_at") if isinstance(source.get("started_at"), str) else None,
        ended_at=ended_at,
...
def ingest_exact_records(records_path: str | Path, db_path: str | Path) -> dict[str, Any]:
    ...
    try:
        for source in records:
            seen += 1
            tool_name = source.get("tool_name")
            tool_counts[str(tool_name) if isinstance(tool_name, str) else "invalid_record"] += 1
            entry = entry_from_record(source, str(jsonl))
            if entry is None:
                unsupported += 1
