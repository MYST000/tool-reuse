...
from ..exact.policy import origin_status, response_status
from ..exact.redact import redact_semantic_text, redact_tool_input
...
def ingest_semantic_records(
    ...
    *,
    ...
    for source in records:
        ...
        origin, origin_reason = origin_status(source, trust_legacy=trust_legacy_origins)
    ...
    try:
        store.delete_source_index(
            str(jsonl), cache_scope, embedder.provider_name, embedder.index_id
        )
        for start in range(0, len(supported), batch_size):
            batch = supported[start : start + batch_size]
            embeddings = embedder.embed_documents(
                [call.semantic_text for _, call in batch]
            )
            ...
            for (source, call), embedding in zip(batch, embeddings):
                ...
                success, status_reason = response_status(
                    tool_response, str(source["tool_name"])
                )
                ended_at = (
                    source.get("ended_at")
                    if isinstance(source.get("ended_at"), str)
                    else None
                )
                ...
                response_text = redact_semantic_text(
                    extract_response_text(safe_tool_response)
                )
                entry = SemanticEntry(
                    cache_scope=cache_scope,
                    record_key=str(
                        source.get("record_key") or sha256_text(call.semantic_text)
                    ),
                    source_path=str(jsonl),
                    call=call,
                    embedding_provider=embedder.provider_name,
                    embedding_model=embedder.index_id,
                    embedding=embedding,
        ...
        return {
            ...
            "database": store.stats(cache_scope),
        }
    finally:
        store.close()
