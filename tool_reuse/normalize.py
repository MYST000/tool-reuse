...
def _normalize_terminal(tool_name: str, tool_input: dict[str, Any]) -> NormalizedToolCall:
...
def _normalize_web_search(tool_name: str, tool_input: dict[str, Any]) -> NormalizedToolCall:
    query = _first_string(tool_input, ("query", "q", "search_query", "keywords", "text")) or stable_json(tool_input)
...
def _normalize_generic(tool_name: str, tool_input: dict[str, Any]) -> NormalizedToolCall:
...
def _looks_like_web_search(tool_name: str, tool_input: dict[str, Any]) -> bool:
    lowered = tool_name.lower()
    if "search" in lowered or lowered in {"web", "browser_search"}:
        return True
    return any(k in tool_input for k in ("search_query", "query", "q")) and "url" not in tool_input
...
def _extract_curl_fingerprint(command: str) -> dict[str, Any] | None:
    ...
    while i < len(curl_tokens):
        ...
        if token.startswith("-X") and len(token) > 2:
            ...
            i += 1
            continue
        header_value = _option_attached_or_next(token, curl_tokens, i, {"-H", "--header"})
        if header_value:
        ...
        if query_value:
            ...
            i += query_value[1]
            continue
        output_value = _option_attached_or_next(token, curl_tokens, i, OUTPUT_VALUE_FLAGS)
        if output_value:
            side_effects.append({"kind": _option_name(token), "target": output_value[0]})
            i += output_value[1]
            continue
        auth_value = _option_attached_or_next(token, curl_tokens, i, {"-u", "--user"})
        if auth_value:
            auth_scope.append({"kind": "basic_auth", "value_sha256": sha256_json(auth_value[0])})
            i += auth_value[1]
            continue
        cookie_value = _option_attached_or_next(token, curl_tokens, i, {"-b", "--cookie", "--cookie-jar"})
        if cookie_value:
            auth_scope.append({"kind": _option_name(token), "value_sha256": sha256_json(cookie_value[0])})
            i += cookie_value[1]
            continue
        user_agent_value = _option_attached_or_next(token, curl_tokens, i, {"-A", "--user-agent"})
        if user_agent_value:
            headers.append(("user-agent", _normalize_header_value(user_agent_value[0])))
            i += user_agent_value[1]
            continue
        value_flag = _option_attached_or_next(token, curl_tokens, i, VALUE_FLAGS)
        if value_flag:
            ignored_options.append(_option_name(token))
            i += value_flag[1]
            continue
...
def _curl_fingerprint_from_parts(
    ...
    all_auth_scope = sorted(auth_scope + url_auth_scope, key=stable_json)
    normalized_headers = sorted(set(headers))
    normalized_secret_headers = sorted(set(secret_headers))
    normalized_options = {k: v for k, v in sorted(options.items()) if v not in (False, None, "", [])}
    return {
        "kind": "curl",
        "method": method.upper(),
        "url": normalized_url,
        ...
        "headers": [{"name": name, "value": value} for name, value in normalized_headers],
        "secret_headers": [
        ...
        "side_effects": sorted(side_effects, key=stable_json),
        "postprocess_signature": postprocess_signature,
        "replay_safe": not side_effects,
    }
...
def canonicalize_url(url: str):
    ...
    if parts.username or parts.password:
        auth_scope.append(
            {
                "kind": "url_userinfo",
                "value_sha256": sha256_json({"username": parts.username, "password": parts.password}),
            }
        )
...
def _shell_side_effects(tail_tokens: list[str]) -> list[dict[str, str]]:
    ...
    for i, token in enumerate(tail_tokens):
        if not _is_redirect(token):
            continue
        target = ""
        if token in {">", ">>", "1>", "1>>", "2>", "2>>", "&>"} and i + 1 < len(tail_tokens):
...
def _option_attached_or_next(
    ...
    short_names = {name for name in option_names if name.startswith("-") and not name.startswith("--")}
...
def _add_header(
    ...
    if SECRET_HEADER_RE.match(normalized_name):
        secret_headers.append((normalized_name, sha256_json(normalized_value)))
    else:
...
def _query_map(query_pairs: list[tuple[str, str]]) -> dict[str, Any]:
    ...
    return {key: values[0] if len(values) == 1 else values for key, values in sorted(grouped.items())}
