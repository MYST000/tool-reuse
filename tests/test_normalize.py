...
def _norm(command: str):
    return normalize_tool_call("terminal", {"command": command, "kind": "TerminalAction"})


class CurlNormalizeTests(unittest.TestCase):
    def test_query_order_matches_exact_key(self) -> None:
        ...
        self.assertEqual(left.fingerprint["url"], "https://example.com/api?a=1&b=2")

    def test_duplicate_query_params_are_preserved(self) -> None:
        result = _norm("curl 'https://example.com/search?tag=b&tag=a&q=x'")

        self.assertEqual(result.fingerprint["url"], "https://example.com/search?q=x&tag=a&tag=b")
        self.assertEqual(result.fingerprint["query"]["tag"], ["a", "b"])

    def test_header_names_and_spaces_are_normalized(self) -> None:
        left = _norm("curl -H 'Accept:   application/json' https://example.com/api")
        right = _norm("curl --header='accept: application/json' https://example.com/api")

        self.assertEqual(left.canonical_key, right.canonical_key)
        self.assertEqual(left.fingerprint["headers"], [{"name": "accept", "value": "application/json"}])

    def test_secret_headers_are_hashed_and_affect_exact_key(self) -> None:
        ...
        self.assertNotEqual(left.canonical_key, different.canonical_key)
    ...
    def test_get_data_params_match_url_query(self) -> None:
        ...
        right = _norm("curl 'https://example.com/search?page=1&q=openai'")
    ...
    def test_output_side_effect_marks_not_replay_safe(self) -> None:
        result = _norm("curl -s https://example.com/file.txt > file.txt && head file.txt")

        self.assertFalse(result.fingerprint["replay_safe"])
        self.assertEqual(result.fingerprint["side_effects"], [{"kind": "shell_redirect", "target": "file.txt"}])
